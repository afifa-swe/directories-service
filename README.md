...
Chunking и поведение пакетов:
- Классы импорта реализуют `WithChunkReading` и метод `chunkSize()` возвращает `500` — это означает, что файл будет разбит на чанки по 500 строк.
- Поскольку импорты также реализуют `ShouldQueue`, Laravel Excel создаёт отдельную queued job для каждого чанка (каждая job обрабатывает 500 строк). Это гарантирует, что большие файлы обрабатываются последовательно и не расходуют память сервера.
- В ТЗ упоминалось «по 100/100» — текущая настройка проекта использует 500 (по коду). Если нужно уменьшить до 100, измените `chunkSize()` на 100 в `app/Imports/*Import.php`.

Дополнительная информация — объявление очереди `imports`:

- Команда `php artisan rabbitmq:declare-imports` создаёт очередь `imports` и (при наличии) привязывает её к настроенному exchange.
- Рекоммендованные варианты вызова команды:
  - Вручную перед первым запуском в новом окружении (локально/CI).
  - В `DatabaseSeeder` через `SetupQueueSeeder` (в репозитории уже добавлен) — удобно для CI и dev окружений.
  - В `docker-entrypoint.sh` или custom deploy-скрипте для автоматического развёртывания.
  - В `composer` post-install-cmd при необходимости (требует доступ к запущенному RabbitMQ).

Команда idempotent — безопасно вызывать несколько раз. См. `app/Console/Commands/DeclareImportsQueue.php` и `database/seeders/SetupQueueSeeder.php`.
